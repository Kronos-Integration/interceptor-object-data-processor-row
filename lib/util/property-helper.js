...
export function getProperty(
  ...
  if (fieldTypePropertyName) {
    if (fieldDefinition.fieldType.hasOwnProperty(fieldTypePropertyName)) {
      if (
        typeof fieldDefinition.fieldType[fieldTypePropertyName] === 'object'
      ) {
  ...
  if (typeof fieldDefinition.fieldType === 'object') {
    ...
    if (fieldTypePropertyName) {
      if (fieldDefinition.fieldType[fieldTypePropertyName]) {
        if (
          fieldDefinition.fieldType[fieldTypePropertyName].hasOwnProperty(
            'severity'
          )
        ) {
...
/**
	 ...
	 * @param fieldDefinition The field_definition
	 */
export function getFieldType(fieldDefinition) {
  const type = fieldDefinition.fieldType;
  return typeof type === 'object' ? fieldDefinition.fieldType.type : type;
}
