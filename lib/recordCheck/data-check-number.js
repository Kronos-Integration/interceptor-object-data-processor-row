...
export function createChecks(fieldDefinition, fieldName) {
  ...
  if (
    ...
    checks = _createChecks(fieldDefinition, fieldName, fieldType);
  }
 ...
 */
function _createChecks(fieldDefinition, fieldName, numberType) {
  let checks = [];

  const decimalSeparator = propertyHelper.getProperty(
    fieldDefinition,
    'decimalSeparator'
  );
  ...
  const defaultValue = parseNumberString(
  ...
  if (typeof defaultValue === 'string') {
    throw {
      ...
      message:
        'The defaultValue in the fieldDefinition is not a valid number in the given format.'
    };
  }
    ...
    if (
      ...
      if (Array.isArray(valueToCheck)) {
        valueToCheck.forEach(function(item, idx, arr) {
          const myNumber = parseNumberString(
            item,
            numberType,
            decimalSeparator
          );
