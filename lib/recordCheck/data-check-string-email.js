...
export function createChecks(fieldDefinition, fieldName) {
  ...
  if (fieldType === 'string' || fieldType === 'email') {
    const defaultValueCheck = createCheckDefaultValue(
      fieldDefinition,
      fieldName
    );
...
function createCheckEmail(fieldDefinition, fieldName) {
  ...
  return functionHelper.getParserCheck(
    errorInfo,
    getEmailIfValid,
    fieldName,
    undefined
...
function createChecksString(fieldDefinition, fieldName) {
  ...
  const minLengthSeverity = propertyHelper.getSeverity(
    fieldDefinition,
    'minLength'
  );
  const maxLengthSeverity = propertyHelper.getSeverity(
    fieldDefinition,
    'maxLength'
  );
