...
import { propertyHelper } from '../util/property-helper';
...
/**
   ...
   */
export function createChecks(fieldDefinition, fieldName) {
  const fieldType = propertyHelper.getFieldType(fieldDefinition);

  if (fieldType === 'date') {
    // This field is a date field. Create the checks
    return _createDateChecks(fieldDefinition, fieldName);
  }
}
 ...
 */
function _createDateChecks(fieldDefinition, fieldName) {
  let checks = [];
  ...
  if (fieldDefinition.defaultValue !== undefined) {
    ...
    if (defaultValue === undefined) {
      throw {
        ...
        message:
          'The defaultValue in the fieldDefinition is not a valid date value.'
      };
    }
  }
  // -----------------------
  ...
  const minDateSeverity = propertyHelper.getSeverity(
    fieldDefinition,
    'minDate'
  );
  const maxDateSeverity = propertyHelper.getSeverity(
    fieldDefinition,
    'maxDate'
  );
...
function parseDateString(dateString) {
  if (dateString) {
    ...
    if (timeZonePart) {
      ...
      timeZonePart = timeZonePart.replace(/^([+-]\d\d)$/, function(match, p1) {
        return p1 + '00';
      });
